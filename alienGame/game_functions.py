...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    number_rows = get_numebr_rows(ai_settings, ship.rect.height, alien.rect.height)
...
def get_numebr_rows(ai_settings, ship_height, alien_height):
    """计算屏幕可容纳多少行外星人"""
    avaliable_space_y = (ai_settings.screen_height - (3 * alien_height) - ship_height)
    numebr_rows = int(avaliable_space_y / (2 * alien_height))
    return numebr_rows
...
def update_aliens(ai_settings, stats, screen, ship, aliens, bullets):
    """检测是否有外星人位于屏幕边缘，并更新外星人群中所有外星人的位置"""
